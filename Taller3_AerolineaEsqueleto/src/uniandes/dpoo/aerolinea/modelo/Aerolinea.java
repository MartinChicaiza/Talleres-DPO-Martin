...
public class Aerolinea
{
    ...
    public Vuelo getVuelo(String codigoRuta, String fechaVuelo) {
        ...
        for (Vuelo v : vuelos) {
            if (v == null) {
                ...
                Ruta r = v.getRuta();
                if (r != null
                    && codigoRuta.equals(r.getCodigoRuta())
    ...
    public Collection<Tiquete> getTiquetes( )
    {
    	...
    	for (Vuelo v : vuelos) {
            if (v !=null) {
                ...
                totalTiquetes.addAll(tiquetesVuelo);}
            }
    ...
    public void programarVuelo(String fecha, String codigoRuta, String nombreAvion) throws Exception
    {
        ...
        if (fecha == null || codigoRuta == null || nombreAvion == null) {
            throw new IllegalArgumentException("fecha, codigoRuta y nombreAvion son obligatorios");
        }
        ...
        Avion avionSeleccionado = buscarAvionPorNombre(nombreAvion);
        if (avionSeleccionado == null) {
            throw new Exception("No existe el avión con nombre: " + nombreAvion);
        }
        if (existeTraslapeDeAvionEnFecha(avionSeleccionado, fecha, rutaSeleccionada)) {
            throw new Exception("El avión '" + nombreAvion + "' ya está programado y se traslapa en la fecha " + fecha);
        }

        Vuelo nuevoVuelo = new Vuelo(fecha, avionSeleccionado, rutaSeleccionada);
    ...
    public int venderTiquetes(String identificadorCliente, String fecha, String codigoRuta, int cantidad) throws VueloSobrevendidoException, Exception
    {
        ...
        if (identificadorCliente == null || fecha == null || codigoRuta == null) {
            throw new IllegalArgumentException("identificadorCliente, fecha y codigoRuta son obligatorios");
        }
        if (cantidad <= 0) {
            return 0;
        }

        Cliente comprador = getCliente(identificadorCliente);
        if (comprador == null) {
            throw new Exception("No existe cliente con identificador: " + identificadorCliente);
        }
        ...
        uniandes.dpoo.aerolinea.modelo.tarifas.CalculadoraTarifas calculadora;
        if (esTemporadaAlta(fecha)) {
            calculadora = new uniandes.dpoo.aerolinea.modelo.tarifas.CalculadoraTarifasTemporadaAlta();
        } else {
            calculadora = new uniandes.dpoo.aerolinea.modelo.tarifas.CalculadoraTarifasTemporadaBaja();
        }

        
        int totalVenta = vuelo.venderTiquetes(comprador, calculadora, cantidad);
        return totalVenta;
    }
    ...
    public String consultarSaldoPendienteCliente(String identificadorCliente)
    {
        ...
        int saldo = c.calcularValorTotalTiquetes();
    ...
    private boolean existeTraslapeDeAvionEnFecha(Avion avion, String fecha, Ruta rutaNueva) {
       
        int iniNueva = toMinutos(rutaNueva.getHoraSalida());
        int finNueva = iniNueva + rutaNueva.getDuracion(); 

        for (Vuelo v : vuelos) {
            if (v != null && fecha.equals(v.getFecha()) && v.getAvion() == avion) {
                Ruta rExistente = v.getRuta();
                int iniExist = toMinutos(rExistente.getHoraSalida());
                int finExist = iniExist + rExistente.getDuracion();

                
                boolean traslape =
                    intervalosTraslapan(iniNueva, finNueva, iniExist, finExist) ||
                    intervalosTraslapan(iniNueva + 1440, finNueva + 1440, iniExist, finExist) ||
                    intervalosTraslapan(iniNueva, finNueva, iniExist + 1440, finExist + 1440);
    ...
    private boolean intervalosTraslapan(int a1, int a2, int b1, int b2) {
        boolean noTraslape = (a2 <= b1) || (b2 <= a1);
        return !noTraslape;
    }

    
    private int toMinutos(String hora) {
        int h = Ruta.getHoras(hora);
        int m = Ruta.getMinutos(hora);
        return h * 60 + m;
    }

    
    private boolean esTemporadaAlta(String fechaYYYYMMDD) {
        if (fechaYYYYMMDD == null) return false;
        String[] p = fechaYYYYMMDD.split("[-/]");
        int mes = 0;
        if (p.length >= 2) {
            try { mes = Integer.parseInt(p[1]); } catch (NumberFormatException e) { mes = 0; }
        }
